--Escreva a definição da função inverteFold :: [a] -> [a] tal que (inverteFold xs ) devolve a 
--lista xs invertida usando a 
...
--Dica: A definição da função inverteFold será inverteFold xs = foldr (\x z -> ) [] xs
